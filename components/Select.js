...
const Select = ({ handleCountry }) => {
  const [selectedPerson, setSelectedPerson] = useState(people[203]);
  ...
  return (
    ...
    >
      <Listbox.Button className="w-full px-3 py-2 text-left border border-gray-300 text-sm text-gray-300 rounded-md focus:outline-none">
        {selectedPerson.name}
      </Listbox.Button>
      <Listbox.Options className="absolute z-20 top-0 inset-x-0 mt-10 bg-white rounded-md focus:outline-none max-h-80 overflow-y-auto">
            ...
            {({ active, selected }) => (
              <li
                className={`px-3 py-3 cursor-pointer ${
                  active ? "bg-gray-200 text-gray-800" : "bg-white text-black"
                }`}
              >
                {person.name}
              </li>
